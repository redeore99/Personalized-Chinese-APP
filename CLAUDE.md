...
- `src/pages/ReadPage.jsx`
  ...
  Segmentation always runs on the simplified text and the resulting boundaries are reused to slice the traditional string for display, because `dictMap` is keyed on simplified headwords and the user's cards are simplified. This relies on `prepare-book.mjs` converting character by character so both scripts have identical length; the code falls back to the simplified text if that ever stops holding. Segmenting the traditional text directly (the original behaviour) left 31% of taps with no definition and halved multi-character recognition. Tokens therefore carry `text` (simplified, used for lookup, known-word matching and card creation) and `shown` (what is rendered).
