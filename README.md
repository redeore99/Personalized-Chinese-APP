...
- Settings now also offers a one-device "Replace This Device With Cloud" recovery action for a stuck browser cache. It rebuilds only that local Dexie cache from Supabase and is meant for cases where another device already has the correct synced library.
